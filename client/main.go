...
type HostInfoHistory struct {
	Id          uint64    `orm:"auto" description:"主键"`
	Host        string    `orm:"size(15)" description:"表示主机地址"`
	Port        uint16    `orm:"default(22)" description:"表示主机ssh端口"`
	MemoryTotal       float64
	MemoryUsed        float64
	MemoryUsedPercent float64
...
func init() {
	dns := fmt.Sprintf("%s:%s@tcp(%s:%d)/%s?charset=utf8&parseTime=true&loc=Local", "root", "123456",
		"192.168.160.134", 3306, "aa")
	//注册数据库，连接对应的生产环境
	orm.RegisterDataBase("default", "mysql", dns)
	orm.RegisterModel(new(HostInfo), new(OsDisk), new(OsDiskHistory),new(HostInfoHistory))
...
func InertMemAllInfo(DB orm.Ormer, ip string) {
	...
	DB.Raw("select id,host,port,memory_total,memory_used,memory_used_percent,memory_cached,"+
		"	swap_total,swap_free,create_time, cpu_count,load1,load5,load15 from host_info where host=?", ip).QueryRows(&hostInfo)
	//将信息表中的详情插入到历史详情表中，并清空详情表对应的记录
	for _, v := range hostInfo {
		InsertMemHistoryInfo(DB,ip,v.MemoryTotal,v.MemoryUsed,v.MemoryUsedPercent,v.MemoryCached,v.SwapTotal,v.SwapFree,
			v.CreateTime,v.CpuCount,v.Load1,v.Load5,v.Load15,v.Port)
		DB.Delete(&HostInfo{Id: v.Id})
	}
	InserMem(DB, ip)
}
...
MemoryCached, SwapTotal, SwapFree float64, createTime time.Time, CpuCount, Load1, Load5, Load15 float64,Port uint16) {
	var hostInfoHistory = HostInfoHistory{
		Host:          Host,
		Port:Port,
		MemoryTotal:       MemoryTotal,
		MemoryUsed:        MemoryUsed,
